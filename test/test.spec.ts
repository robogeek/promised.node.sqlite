...
// TODO Test all three params structures with a variety of value types

test.serial('Use db.run with single item in params', async t => {
 ...
 * Querying all rows with all() method
 */
test.serial('Querying all rows with all() method #2', async t => {
  const sql = `SELECT DISTINCT Name name FROM playlists
               ORDER BY name`
  const db = await open('./assets/chinook.db')
  const rows = await db.all(sql)
